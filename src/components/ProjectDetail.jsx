    ...
    <section className='container mx-auto p-6'>
      ...
      <div className='mt-6 space-y-4'>
        {project.content.map((item, index) => {
          ...
          if (type === "heading")
            return (
              <h2 key={index} className='text-2xl font-semibold text-gray-300'>
                {restText}
              </h2>
            );
          if (type === "para")
          ...
          if (type === "list")
            return (
              <li key={index} className='text-gray-400 list-disc list-inside'>
                {restText}
              </li>
            );
        })}
      ...
      {/* React Flow Diagram */}
      <div className='mt-10 h-[400px] text-md text-blue-500 bg-blue-500/10 rounded-lg'>
        <ReactFlow nodes={project.initNodes} edges={project.initEdges} fitView>
          <Background color='text-gray-400' variant='cross' />
        </ReactFlow>
      </div>
    </section>
  );
